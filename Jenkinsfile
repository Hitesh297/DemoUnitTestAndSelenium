    ...
    stages {
        stage('Build') { 
            steps { 
			   ...
			   & 'packages\\OpenCover.4.7.922\\tools\\OpenCover.Console.exe' -register:user -filter:"+[*]*" -target:"C:\\Program Files (x86)\\Microsoft Visual Studio\\2017\\Community\\Common7\\IDE\\CommonExtensions\\Microsoft\\TestWindow\\vstest.console.exe" -targetargs:"/noshadow UnitTestProject1\\bin\\Release\\UnitTestProject1.dll  /ResultsDirectory:result /logger:trx" -output:"CodeCoverage\\OpenCover.xml" 
