    ...
    stages {
        stage('Build') { 
            steps { 
			   ...
			   bat '%WORKSPACE%\\packages\\OpenCover.4.7.922\\tools\\OpenCover.Console.exe -register:%user% -target:"C:\\Program Files (x86)\\Microsoft Visual Studio\\2017\\Community\\Common7\\IDE\\CommonExtensions\\Microsoft\\TestWindow\\vstest.console.exe" -targetargs:"%WORKSPACE%\\UnitTestProject1\\bin\\Release\\UnitTestProject1.dll /ResultsDirectory:""%WORKSPACE%\\Testresult"" /logger:trx" -output:"%WORKSPACE%\\CodeCoverage\\OpenCover.xml" -mergebyhash'
            }
        }
        stage('Test'){
            steps {
